...
# welcome message and rules
  ...
  def play
    ...
    loop do                # infinite loop
      ...
      black_pegs = @board.feedback_and_display(code, guessed_code)

      if @board.guess_correct?(black_pegs)
        victory
        break
      elsif max_guess_attempts?(guess_attempts)
        defeat
        break
      end
    end
  ...
  def max_guess_attempts?(guess_attempts)
...
class Player
  ...
  def create_code
  # random_generation (could be alternatively by human input)    
  code = []
    4.times do
      code << rand(1..6).to_s
    end
